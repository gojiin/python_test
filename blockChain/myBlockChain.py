...
class Block:
    ...
    def __init__(self, index, previousHash, timestamp, data, currentHash, proof, identityNumber):
        self.index = index
...
class txData:
    ...
    def __init__(self, commitYN, sender, amount, receiver, uuid, identityNumber):
...
def generateGenesisBlock():
    ...
    timestamp = time.time()
    ##여기다가 주민번호 함수든 뭐든 써 랜덤값으로 뽑든지....
    print("time.time() => %f \n" % timestamp)
    tempHash = calculateHash(0, '0', timestamp, "Genesis Block", 0, 0)
    print(tempHash)
    return Block(0, '0', timestamp, "Genesis Block",  tempHash, 0, 0)

def calculateHash(index, previousHash, timestamp, data, proof, identityNumber):
    value = str(index) + str(previousHash) + str(timestamp) + str(data) + str(proof) + str(identityNumber)
    sha = hashlib.sha256(value.encode('utf-8'))
    return str(sha.hexdigest())

def calculateHashForBlock(block):
    return calculateHash(block.index, block.previousHash, block.timestamp, block.data, block.proof, block.identityNumber)

def getLatestBlock(blockchain):
    return blockchain[len(blockchain) - 1]

def generateNextBlock(blockchain, blockData, timestamp, proof, identityNumber):
    previousBlock = getLatestBlock(blockchain)
    nextIndex = int(previousBlock.index) + 1
    nextTimestamp = timestamp
    nextIdentityNumber = identityNumber
    nextHash = calculateHash(nextIndex, previousBlock.currentHash, nextTimestamp, blockData, proof, nextIdentityNumber)
    # index, previousHash, timestamp, data, currentHash, proof
    return Block(nextIndex, previousBlock.currentHash, nextTimestamp, blockData, nextHash,proof, nextIdentityNumber)

def writeBlockchain(blockchain):
    blockchainList = []

    for block in blockchain:
        blockList = [block.index, block.previousHash, str(block.timestamp), block.data, block.currentHash,block.proof, block.identityNumber ]
...
def writeTx(txRawData):
    ...
    for txDatum in txRawData:
        txList = [txDatum.commitYN, txDatum.sender, txDatum.amount, txDatum.receiver, txDatum.uuid, txDatum.identityNumber]
...
def getTxData():
    ...
    if len(importedTx) > 0 :
        for i in importedTx:
            print(i.__dict__)
            transaction = "["+ i.uuid + "]" "UserID " + i.sender + " sent " + i.amount + " bitTokens to UserID " + i.receiver + ". " + i.identityNumber + "identityNumber"#
            print(transaction)
...
def mineNewBlock(difficulty=g_difficulty, blockchainPath=g_bcFileName):
    ...
    while not newBlockFound:
        newBlockAttempt = generateNextBlock(blockchain, strTxData, timestamp, proof, identityNumber)
...
def isSameBlock(block1, block2):
    if str(block1.index) != str(block2.index):
        ...
        return False
    elif str(block1.identityNumber) != str(block2.identityNumber):
        return False
    return True
...
def newtx(txToMining):
    ...
    for line in txToMining:
        tx = txData(0, line['sender'], line['amount'], line['receiver'], uuid.uuid4(), line['identityNumber'])
        newtxData.append(tx)
...
#                break
    ...
    for line in bcToValidate:
        # print(type(line))
        # index, previousHash, timestamp, data, currentHash, proof
        block = Block(line['index'], line['previousHash'], line['timestamp'], line['data'], line['currentHash'], line['proof'], line['identityNumber'])
        ...
        # select longest chain
            ...
            for block in bcToValidateForBlock:
                blockList = [block.index, block.previousHash, str(block.timestamp), block.data,
                             block.currentHash, block.proof, block.identityNumber]
                blockchainList.append(blockList)
            with open(g_bcFileName, "w", newline='') as file:
                writer = csv.writer(file)
                writer.writerows(blockchainList)
        ...
        for key, value in g_nodeList.items():
            ...
            if res.status_code == 200 :
                ...
                for line in tmpbcData:
                    # print(type(line))
                    # index, previousHash, timestamp, data, currentHash, proof
                    block = [line['index'], line['previousHash'], line['timestamp'], line['data'],line['currentHash'], line['proof'], line['identityNumber']]
